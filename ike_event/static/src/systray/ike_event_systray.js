...
export class IkeEventSystray extends Component {
    ...
    async broadcastEventSuppliersDeleted(payload) {
        return this._triggerBusEvent("EVENT_SUPPLIERS_DELETED", payload);
    }
                    ...
                    if (states_to_notify.includes(item.state)) {
                        ...
                        switch (item.state) {
                            case 'accepted':
                                ...
                                message_options.title += _t("Service Accepted");
                                message_options.type = "success";
                                break;
                            case 'rejected':
                                message = `${supplier_name}`;
                                message_options.title += _t("Service Rejected");
                                message_options.type = "warning";
                                break;
                            case 'cancel':
                                message = `${supplier_name}`;
                                message_options.title += _t("Service Cancelled");
                                message_options.type = "danger";
                                message_options.sticky = true;
                                break;
                            case 'cancel_supplier':
                                message = `${supplier_name}`;
                                message_options.title += _t("Service Cancelled by Supplier");
                                message_options.type = "danger";
                                message_options.sticky = true;
                                break;
                            default:
