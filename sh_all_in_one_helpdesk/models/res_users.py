...
class ResUsers(models.Model):
    ...
    sh_portal_user = fields.Boolean(
        string='Portal',
        compute='_compute_sh_portal_user',
        search='_search_sh_portal_user',
    )
    sh_portal_user_access = fields.Selection(
        [('user', 'Portal Support User'), ('manager', 'Portal Manager')],
    ...
    # Notification
    sh_create_new_ticket_notification = fields.Boolean(
        string='New Ticket Notification?'
    )
    sh_assign_ticket_notification = fields.Boolean(
    ...
    def SELF_READABLE_FIELDS(self):
        return super().SELF_READABLE_FIELDS + [
            'sign',
            'sh_create_new_ticket_notification',
            'sh_assign_ticket_notification',
            'sh_ticket_done_notification',
            'sh_ticket_cancel_notification',
        ]
    ...
    def systray_get_notifications(self):
        notifications = self.env['sh.user.push.notification'].search(
            [('user_id', '=', self.env.uid)], limit=25, order='msg_read,id desc'
        )
        unread_notifications = self.env['sh.user.push.notification'].search(
            [('user_id', '=', self.env.uid), ('msg_read', '=', False)]
        )
        data_notifications = []
    ...
    def systray_get_all_notifications(self):
        notifications = self.env['sh.user.push.notification'].search(
            [('user_id', '=', self.env.uid)], order='msg_read,id desc'
        )
        unread_notifications = self.env['sh.user.push.notification'].search(
            [('user_id', '=', self.env.uid), ('msg_read', '=', False)]
        )
...
class Http(models.AbstractModel):
    ...
    def session_info(self):
        ...
        user = self.env.user
        info["sign"] = user.sign
        info["sh_create_new_ticket_notification"] = (
            user.sh_create_new_ticket_notification
        )
