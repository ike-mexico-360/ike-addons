...
class TicketAlarm(models.Model):
    ...
    def _run_ticket_reminder(self):
        if self.env.company.sh_display_ticket_reminder:
            alarm_ids = self.env['sh.ticket.alarm'].search([])
            if alarm_ids:
                for alarm in alarm_ids:
                    ticket_ids = self.env['sh.helpdesk.ticket'].search([('sh_ticket_alarm_ids','in',[alarm.id])])
                    if ticket_ids:
                            ...
                            if deadline_date and deadline_date != False:
                                if alarm.type == 'popup':
                                    ...
                                    if fields.Date.today() == deadline_date.date() and deadline_date.hour == now.hour and deadline_date.minute == now.minute:
                                        ...
                                        model_href = str(self.get_base_url()) + '/web#id='+str(ticket.id)+'&model=sh.helpdesk.ticket&view_type=form'
                                        message += "<a href=" + \
                                            str(
                                    ...
                                    if fields.Date.today() == deadline_date.date() and deadline_date.hour == now.hour and deadline_date.minute == now.minute:
                                        ...
                                        if reminder_template and len(email_formatted) > 0:
                                            ...
                                            reminder_template.send_mail(ticket.id, force_send=True,email_values=email_values)
