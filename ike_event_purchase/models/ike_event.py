...
class IkeEvent(models.Model):
    ...
    def x_action_create_purchases(self):
        self.ensure_one()
        ...
        if self.stage_ref == 'completed' and len(self.x_purchase_ids) == 0:
            ...
            for supplier_line in self.selected_supplier_ids:
                product_ids = supplier_line.supplier_link_id.supplier_product_ids.filtered(lambda x: x.product_id)
            ...
            for purchase_vals in grouped_purchase_by_suppliers.values():
                self.env['purchase.order'].with_context(dict(ike_event_purchase=True)).create(purchase_vals)
